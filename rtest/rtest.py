...
def doDiff(OUTFILE, testname, subtest_index, fmt):
  ...
  if F in ["ps", "ps2"]:
    ...
    with open(FILE1, "rt", encoding="utf-8") as src:
      with open(TMPFILE1, "wt", encoding="utf-8") as dst:
        done_setup = False
        for line in src:
          if done_setup:
            dst.write(line)
    ...
    with open(FILE2, "rt", encoding="utf-8") as src:
      with open(TMPFILE2, "wt", encoding="utf-8") as dst:
        done_setup = False
        for line in src:
          if done_setup:
            dst.write(line)
